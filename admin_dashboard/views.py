...
def dashboard_page(request):
    ...
    ---------------------------------------
    Dashboard data is calculated from the
    existing NAFI product, order and user models.
    """
    ...
    active_products = ProductsModel.objects.filter(
        is_available=True
    ).count()
    ...
    # ----------------------------------------------------------
    # Product stock is maintained from ProductSize records.
    # Therefore calculate stock using the size-wise inventory.
    # ----------------------------------------------------------

    products_with_sizes = ProductsModel.objects.prefetch_related(
        "sizes"
    )
    ...
    delivered_order_queryset = Order.objects.filter(
        status="Delivered"
    )

    total_revenue = (
        delivered_order_queryset.aggregate(
    ...
    # CUSTOMERS
    #
    # Registered customers are taken from the custom User model.
    # Guest orders are counted separately for dashboard display.
    # ==========================================================

    User = get_user_model()

    registered_customers = User.objects.count()
    ...
    #
    # Only products from DELIVERED orders are counted.
    # ==========================================================
    ...
    # ----------------------------------------------------------
    # Attach actual product object/image where available.
    # ----------------------------------------------------------
    ...
    top_products_map = {
        product.id: product
        for product in ProductsModel.objects.filter(
    ...
    for item in top_selling_products:
        ...
        item["product_object"] = (
            top_products_map.get(product_id)
            if product_id
            else None
        )
    ...
    # REVENUE CHART
    #
    # Default = current year monthly revenue.
    ...
    revenue_map = {
        item["month"].month: float(item["revenue"] or 0)
